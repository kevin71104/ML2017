...
### parsing and Data pre-processing
	...
	##Do the stacking by adding 3 dataframes 'negbi', 'gb', 'xgb' which store the training prediction
	...
	for train_index, val_index in kf.split(sj_train): #The index will be split into [train_index] and [val_index] 
		...
		sj_gb_model = gradient_boosting(X_train.drop(['negbi','gb','xgb'],axis = 1)
			,X_val.drop(['negbi','gb','xgb'],axis = 1))	
		predictions_gb = sj_gb_model.predict(X_val.drop(['total_cases','negbi','gb','xgb'],axis = 1)).astype(int)
		
		###(3)xgboost method
		dtrain = xgb.DMatrix(X_train.drop(['total_cases','negbi','gb','xgb'],axis = 1), X_train['total_cases'])
		dval = xgb.DMatrix(X_val.drop(['total_cases','negbi','gb','xgb'],axis = 1))
	...
	for train_index, val_index in kf.split(iq_train):
		...
		###(2)gradient boosting method
		iq_gb_model = gradient_boosting(X_train.drop(['negbi','gb','xgb'],axis = 1),
			X_val.drop(['negbi','gb','xgb'],axis = 1))
		predictions_gb = iq_gb_model.predict(X_val.drop(['total_cases','negbi','gb','xgb'],axis = 1)).astype(int)
	...
	sj_predictions_gb = sj_gb_model.predict(sj_test.drop(['negbi','gb','xgb'],axis = 1)).astype(int)
	##(3)xgb prediction
	dtest = xgb.DMatrix(sj_test.drop(['negbi','gb','xgb'],axis = 1))
	...
	iq_predictions_gb = iq_gb_model.predict(iq_test.drop(['negbi','gb','xgb'],axis = 1)).astype(int)
	
	dtest = xgb.DMatrix(iq_test.drop(['negbi','gb','xgb'],axis = 1))
	...
	submission.to_csv("./data/stacking_3_less_feature.csv")
